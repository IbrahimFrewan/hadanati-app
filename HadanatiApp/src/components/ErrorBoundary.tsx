...
/**
 * Catches render-time errors in any wrapped screen so a single screen
 * throwing can no longer crash the whole app. Shows a friendly fallback
 * (with the underlying message, useful for diagnosing) and a retry button.
 */
export class ErrorBoundary extends React.Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: { componentStack: string }) {
    // Surface in dev logs; never rethrow.
...
/** Wraps a screen component in an ErrorBoundary. Call at module scope so the
 *  returned component reference stays stable across re-renders (otherwise React
 *  Navigation would remount the screen on every parent render). */
