...
export function CheckoutScreen({ navigation }: any) {
  ...
  const Row = ({ label, value, bold }: { label: string; value: string; bold?: boolean }) => (
    <View style={{ flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between', paddingVertical: 7 }}>
      <Text style={{ fontFamily: bold ? F.displayBold : F.body, fontSize: bold ? 15 : 13.5, fontWeight: bold ? '800' : '500', color: bold ? C.ink : C.mut, textAlign: isRTL ? 'right' : 'left' }}>{label}</Text>
      <Text style={{ fontFamily: bold ? F.displayBold : F.body, fontSize: bold ? 15 : 13.5, fontWeight: bold ? '800' : '500', color: bold ? C.dgreen : C.ink, textAlign: isRTL ? 'right' : 'left' }}>{value}</Text>
    </View>
  );
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.page }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 22 }}>
        ...
        <View style={{ flexDirection: isRTL ? 'row-reverse' : 'row', alignItems: 'center', gap: 7, backgroundColor: '#fbeede', borderRadius: 12, padding: 13, marginBottom: 18 }}>
          ...
          <Text style={{ fontSize: 12.5, fontWeight: '600', color: '#9a6310', flex: 1, textAlign: isRTL ? 'right' : 'left' }}>
        ...
        <View style={{ borderWidth: 1, borderColor: C.line, borderRadius: 16, padding: 15, marginBottom: 18 }}>
          <View style={{ flexDirection: isRTL ? 'row-reverse' : 'row', gap: 12, marginBottom: 13 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontFamily: F.displayBold, fontSize: 16, fontWeight: '700', color: C.ink, marginBottom: 3, textAlign: isRTL ? 'right' : 'left' }}>{n.name}</Text>
              <Text style={{ fontSize: 12, color: C.mut, textAlign: isRTL ? 'right' : 'left' }}>{t(lang, type)} · {childNames}</Text>
            </View>
          </View>
          <View style={{ flexDirection: isRTL ? 'row-reverse' : 'row', alignItems: 'center', gap: 8, paddingTop: 11, borderTopWidth: 1, borderTopColor: C.line }}>
            ...
            <Text style={{ fontSize: 12.5, color: C.mut, textAlign: isRTL ? 'right' : 'left' }}>{store.draft.dates || 'Upcoming'}</Text>
          </View>
        </View>

        {/* Payment methods */}
        <Text style={{ fontFamily: F.displayBold, fontSize: 16, fontWeight: '700', color: C.ink, marginBottom: 11, textAlign: isRTL ? 'right' : 'left' }}>{t(lang, 'paymentMethod')}</Text>
        ...
        <View style={{ flexDirection: isRTL ? 'row-reverse' : 'row', gap: 9, marginBottom: 18 }}>
          <View style={{ flex: 1, flexDirection: isRTL ? 'row-reverse' : 'row', alignItems: 'center', gap: 8, borderWidth: 1, borderColor: C.line, borderRadius: 12, paddingHorizontal: 13, height: 46 }}>
            <Icon name="tag" size={17} color={C.mut} />
            <Text style={{ fontSize: 13, color: C.mut, textAlign: isRTL ? 'right' : 'left' }}>{t(lang, 'promoCode')}</Text>
          </View>
        ...
        <View style={{ flexDirection: isRTL ? 'row-reverse' : 'row', gap: 7, marginTop: 14, alignItems: 'flex-start' }}>
          <Icon name="lock" size={15} color={C.mut} />
          <Text style={{ fontSize: 11, color: C.mut, lineHeight: 17, flex: 1, textAlign: isRTL ? 'right' : 'left' }}>{t(lang, 'pciNote')}</Text>
        </View>
      </ScrollView>
